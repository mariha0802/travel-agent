...
def initialize_agent(api_key: str):
    ...
    if not api_key or not api_key.strip():
        return (
            ...
            gr.update(value="Please enter a valid OpenAI API key."),
        )

    try:
        os.environ["OPENAI_API_KEY"] = api_key.strip()
        app_travel_agent = build_graph_one_tool(tools, api_key=api_key.strip())
        return (
            gr.update(value=""),
            gr.update(visible=False),
            gr.update(visible=True),
            gr.update(value="✅ Assistant unlocked. You can now chat."),
        )
    except Exception as exc:
        app_travel_agent = None
        return (
            ...
            gr.update(visible=True),
            gr.update(visible=False),
            gr.update(value=f"❌ Invalid OpenAI key. Please check your key and try again. Details: {exc}"),
        )


with gr.Blocks(title="✈️ LangGraph AI Travel Agent 🌍") as demo:
    gr.Markdown("# ✈️ LangGraph AI Travel Agent 🌍")

    auth_panel = gr.Column()
    with auth_panel:
        gr.Markdown("### Enter your OpenAI API key to unlock the assistant")
        api_key_input = gr.Textbox(
            type="password",
            label="OpenAI API key",
            placeholder="sk-...",
        )
        submit_key = gr.Button("Unlock assistant")
    ...
    with chat_panel:
        gr.ChatInterface(
            ...
            textbox=gr.Textbox(
                placeholder="Plan your trip! Ask about attractions, flights, hotels...",
                container=False,
                scale=7,
            ),
            title="✈️ LangGraph AI Travel Agent 🌍",
            description="Your stateful travel assistant for flights, hotels, news, and itinerary planning.",
            examples=[
                "I want to visit Paris from Toronto from 2027-07-01 to 2027-07-08. Find flights and hotel ideas.",
                "What are the latest travel advisories for New York?",
                "Find me the cheapest flights from Toronto to London for next month."
            ],
        )
    ...
    submit_key.click(
        ...
        outputs=[api_key_input, auth_panel, chat_panel, auth_status],
    )
